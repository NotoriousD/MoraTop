...
class Container {
	...
	static public function share( array $names_or_instances ) {
		...
		collect( $names_or_instances )->each( function ( $name_or_instance ) use ( $injector ) {
	...
	static public function alias( array $aliases ) {
		...
		collect( $aliases )->each( function ( $alias, $original ) use ( $injector ) {
	...
	static public function delegate( array $delegated ) {
		...
		collect( $delegated )->each( function ( $instantiator, $class_name ) use ( $injector ) {
